...
{
    public partial class ReportEvaluation : Page
    {
        SqlCommand cmd;
        SqlConnection con;
        SqlDataAdapter da;
        ...
        private void populateMarks()
        {
            foreach (GridViewRow row in GridView1.Rows)
            {
                ...
                con = new SqlConnection(connectionString);
                con.Open();
                cmd = new SqlCommand("select Marks from ReportMarks where Group_Id = @Group_Id and MarksFor = @MarksFor and CLO_Id = @CLO_Id and Supervisor_Id = @Supervisor_Id", con);
        ...
        protected void GridView1_RowUpdating(object sender, GridViewUpdateEventArgs e)
        {
            ...
            {
                //lbl.Text = GridView1.Rows[e.RowIndex].Cells[5].Text;
                ...
                if (Convert.ToDecimal(value) >= 0 && Convert.ToDecimal(value) <= 5)
                {
                    ...
                    cmd = new SqlCommand("INSERT INTO ReportMarks (Group_Id,CLO_Id,Marks,Supervisor_Id,MarksFor) VALUES (@Group_Id,@CLO_Id,@Marks,@Supervisor_Id,@MarksFor)", con);
                    cmd.Parameters.AddWithValue("@Group_Id", SqlDbType.VarChar).Value = groupNumber.SelectedValue;
                    cmd.Parameters.AddWithValue("@Supervisor_Id", SqlDbType.VarChar).Value = E;
                    cmd.Parameters.AddWithValue("@CLO_Id", SqlDbType.Int).Value = cloID;
                    cmd.Parameters.AddWithValue("@MarksFor", SqlDbType.VarChar).Value = reports.SelectedItem.ToString();
                ...
                else
                {
                    ...
                    //string message1 = "please marks should be from 1 to 5";
        ...
        private bool IsMarksExist(int cloID)
        {
            con = new SqlConnection(connectionString);
            con.Open();
            cmd = new SqlCommand("select '#' from ReportMarks where Group_Id = @Group_Id and MarksFor = @MarksFor and CLO_Id = @CLO_Id and Supervisor_Id = @Supervisor_Id", con);
            cmd.Parameters.AddWithValue("@Group_Id", SqlDbType.VarChar).Value = groupNumber.SelectedValue;
            cmd.Parameters.AddWithValue("@Supervisor_Id", SqlDbType.VarChar).Value = E;
            cmd.Parameters.AddWithValue("@CLO_Id", SqlDbType.Int).Value = cloID;
            cmd.Parameters.AddWithValue("@MarksFor", SqlDbType.VarChar).Value = reports.SelectedItem.ToString();
